...
ArenaHeader::checkSynchronizedWithFreeList() const
{
     ...
     * checks in this case.
     */
    if (IsBackgroundFinalized(getAllocKind()) && zone->runtimeFromAnyThread()->gcHelperState.onBackgroundThread())
        return;
...
ChunkPool::wantBackgroundAllocation(JSRuntime *rt) const
{
     ...
     * of them.
     */
    return rt->gcHelperState.canBackgroundAllocate() &&
           emptyCount == 0 &&
  ...
  public:
    ...
    ~AutoMaybeStartBackgroundAllocation() {
        if (runtime) {
            ...
            runtime->gcHelperState.startBackgroundAllocationIfIdle();
        }
    }
};
...
static Chunk *
PickChunk(Zone *zone, AutoMaybeStartBackgroundAllocation &maybeStartBackgroundAllocation)
{
    JSRuntime *rt = zone->runtimeFromAnyThread();
                                    ...
                                    AutoMaybeStartBackgroundAllocation &maybeStartBackgroundAllocation)
{
    ...
    if (*bfs != BFS_DONE) {
        ...
        if (*bfs == BFS_RUN) {
            JS_ASSERT(!*al->cursor);
            chunk = PickChunk(zone, maybeStartBackgroundAllocation);
            if (!chunk) {
                /*
...
#ifdef JS_THREADSAFE
    JS_ASSERT(!fop->runtime()->gcHelperState.sweeping());
#endif
    ...
    for (;;) {
        ...
        if (cx->isJSContext()) {
            ...
            for (bool secondAttempt = false; ; secondAttempt = true) {
                ...
                cx->asJSContext()->runtime()->gcHelperState.waitBackgroundSweepEnd();
            }
        } else {
#ifdef JS_THREADSAFE
            /*
...
GCHelperState::init()
{
    ...
    if (!WorkerThreadState().ensureInitialized())
        return false;
#endif /* JS_THREADSAFE */
...
GCHelperState::state()
{
    JS_ASSERT(rt->currentThreadOwnsGCLock());
    return state_;
}
    ...
    switch (rt->gcIncrementalState) {
      ...
      case SWEEP:
...

        {
            ...
            rt->gcHelperState.waitBackgroundSweepOrAllocEnd();
        }
        break;
                        ...
                        JSGCInvocationKind gckind)
{
    ...
    switch (rt->gcIncrementalState) {
      ...
      case SWEEP: {
        ...
        if (rt->gcSweepOnBackgroundThread)
            rt->gcHelperState.startBackgroundSweep(gckind == GC_SHRINK);
...
JS::ShrinkGCBuffers(JSRuntime *rt)
{
    ...
    if (!rt->useHelperThreads())
        ...
        rt->gcHelperState.startBackgroundShrink();
}
...
{
    rt->gcHelperState.waitBackgroundSweepEnd();
}
