...
GCRuntime::pickChunk(Zone *zone, AutoMaybeStartBackgroundAllocation &maybeStartBackgroundAllocation)
{
    ...
    if (rt->gc.wantBackgroundAllocation())
...
void
GCRuntime::startBackgroundAllocationIfIdle()
{
    helperState.startBackgroundAllocationIfIdle();
}
...
#ifdef JS_THREADSAFE
    JS_ASSERT(!fop->runtime()->gc.helperState.sweeping());
#endif
    ...
    for (;;) {
        ...
        if (cx->isJSContext()) {
            ...
            for (bool secondAttempt = false; ; secondAttempt = true) {
                ...
                cx->asJSContext()->runtime()->gc.helperState.waitBackgroundSweepEnd();
            }
        } else {
#ifdef JS_THREADSAFE
            /*
...
GCHelperState::state()
{
    JS_ASSERT(rt->gc.currentThreadOwnsLock());
    return state_;
}
    ...
    switch (incrementalState) {
      ...
      case SWEEP:
...

        {
            ...
            helperState.waitBackgroundSweepOrAllocEnd();
        }
        break;
