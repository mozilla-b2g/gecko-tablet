...
mv *.linux-x86_64.tar.bz2       $HOME/artifacts/firefox.linux-x86_64.tar.bz2
mv *.linux-x86_64.json          $HOME/artifacts/firefox.linux-x86_64.json
for name in common cppunittest reftest mochitest xpcshell web-platform; do
    mv *.$name.tests.zip          $HOME/artifacts/firefox.$name.tests.zip ;
done
mv *.test_packages.json         $HOME/artifacts/firefox.test_packages.json
mv *.crashreporter-symbols.zip  $HOME/artifacts/firefox.crashreporter-symbols.zip
